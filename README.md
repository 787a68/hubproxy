...
```bash
curl -fsSL https://raw.githubusercontent.com/787a68/hubproxy/main/install.sh | VERSION=2026.07.06 sh
```
...
通配符示例：`baduser/*`、`*/malicious-repo`、`baduser/malicious-repo`
...
| 配置文件 | 环境变量 | 默认值 | 说明 |
| --- | --- | --- | --- |
| `[tokenCache] enabled` | `CACHE` | `true` | 启用缓存（同时控制 Token 和 Manifest） |
| `[tokenCache] defaultTTL` | `CACHE_TTL` | `20m` | 缓存 TTL，Go duration 格式 |
